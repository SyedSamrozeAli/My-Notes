...
int main()
{
    ...
    // h.root = new node(5);
    // h.root->left = new node(53);
    // h.root->left->left = new node(20);
    // h.root->left->right = new node(60);
    // h.root->right = new node(55);

    h.levelOrderTraversal();

    h.root = h.Delete(h.root);
    // h.root = h.MaxHeapifyDown(h.root);
    cout << endl;
    h.levelOrderTraversal();
}